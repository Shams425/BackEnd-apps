<?php  
	...
	if (isset($_POST['sub'])) {
		...
		for ($i=0; $i < $count; $i++) { 
			$date=date("d-m-y");
			$que="INSERT INTO 
							class_result(roll_no,course_code,subject_code,semester,total_marks,obtain_marks,result_date)
					  VALUES 
							('".$_POST['roll_no'][$i]."','".$_POST['course_code'][$i]."',
							 '".$_POST['subject_code'][$i]."','".$_POST['semester'][$i]."','".$_POST['total_marks'][$i]."',
							 '".$_POST['obtain_marks'][$i]."','$date'
							)";
							...
							<table class="w-100 table-elements table-three-tr" cellpadding="3">
								...
								<?php  
									...
									if (isset($_POST['submit'])) {
										...
										$que=
                    "SELECT 
                      student_info.roll_no, first_name, middle_name, last_name, student_courses.semester, student_courses.course_code, subject_code 
										FROM
										  student_courses 
										INNER JOIN 
									    student_info 
										ON 
										  student_info.roll_no = student_courses.roll_no 
										WHERE 
										  student_courses.course_code = '$course_code' AND student_courses.roll_no = '$roll_no'";

									  $run = mysqli_query($con,$que);
									  while ($row=mysqli_fetch_array($run)) {
										  $count++;
										  echo"<form action='add-single-student-results.php' method='post'>";
											echo "<tr>";
												echo"<td> $i++ ?></td>";
												...
												echo"<input type='hidden' name='total_marks[]' value='100' >";
												echo"<td><input type='text' name='obtain_marks[]' placeholder='Plz Enter Obtain Marks' class='form-control' required/></td>";
