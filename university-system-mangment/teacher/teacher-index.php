								...
								<div class="collapse show mt-2" id="collapsetwo">
									<table class="w-100 table-elements table-three-tr"cellpadding="2">
										...
										<?php  
											...
											$query=
                      "SELECT 
                        salary_id,basic_salary,medical_allowance,hr_allowance,scale,Date(paid_date) AS paid_date,total_amount 
                      FROM 
                        teacher_salary_allowances 
                      INNER JOIN 
                        teacher_salary_report on teacher_salary_allowances.teacher_id = teacher_salary_report.teacher_id 
                      WHERE 
                        teacher_salary_allowances.teacher_id = '$teacher_id'";
					...
					<div class="col-md-12 mt-4">
						<table class="w-100 table-striped table-hover table-dark"cellpadding="2" >
							...
							<?php
								$query=
                "SELECT 
                  month(attendance_date) AS attendance_date,sum(attendance) AS present 
                FROM 
                  teacher_attendance  
                WHERE 
                  teacher_id='$teacher_id' group by month(attendance_date) ";
